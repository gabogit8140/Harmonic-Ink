...
const App: React.FC = () => {
  ...
  const { coefficients, targetPoints, letterBreaks, isComputing, compute, reset, optimalHarmonics } = useFourier();

  // Reset when text/font changes
  useEffect(() => {
    setMode('preview');
    reset();
    setNumHarmonics(500); // Default fallback
  }, [text, selectedFont, reset]);

  // Apply optimal harmonics when calculated
  useEffect(() => {
  ...
  const handleConfirmValidation = () => {
      // Preserve the user's selected complexity from validation phase
          ...
          <div className="mt-6 px-4 py-4 md:px-8 md:py-3 bg-white/[0.02] rounded-3xl border border-white/5 text-[9px] text-slate-500 font-mono flex flex-col md:flex-row justify-between md:justify-center items-center gap-4 md:gap-12 uppercase tracking-[0.2em] backdrop-blur-sm text-center">
            {mode === 'preview' ? (
                <span className="flex items-center gap-2 text-emerald-400">
                    ...
                    PREVIEW MODE: Validate Font Style
                </span>
            ) : mode === 'validating' ? (
                <span className="flex items-center gap-2 text-cyan-400">
                    <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse"></span>
                    VALIDATION MODE: Confirm Path Alignment
                </span>
            ) : (
                <span className="flex items-center gap-2">
                <span className={`w-1.5 h-1.5 rounded-full ${!isPaused ? 'bg-cyan-400' : 'bg-amber-400'}`}></span>
                {!isPaused ? 'Tracing active' : 'Trace Complete / Paused'}
                </span>
            )}
            
            <div className="flex gap-8 md:gap-12">
              <span>Vectors: {mode === 'tracing' ? numHarmonics : (mode === 'validating' ? optimalHarmonics : 'N/A')}</span>
