        ...
        <header className="w-full flex flex-col items-center gap-8 py-8 relative z-10">
            ...
            <div className="flex flex-row items-center justify-center gap-5 md:gap-6 relative w-full">
                <div className="flex flex-row items-center justify-center gap-5 md:gap-6">
                    ...
                    <div className="flex flex-col items-start pr-12 md:pr-0">
                        <h1 className="text-3xl md:text-5xl lg:text-6xl font-black bg-gradient-to-br from-white via-slate-200 to-slate-500 bg-clip-text text-transparent tracking-tighter uppercase font-sans leading-none">
                            Harmonic Ink
                        </h1>
                ...
                <button
                    onClick={onOpenInfo}
                    className="absolute right-0 top-1/2 -translate-y-1/2 flex items-center gap-3 px-3 md:px-5 py-2.5 bg-slate-900/80 hover:bg-slate-800 text-slate-400 hover:text-white transition-all rounded-full border border-white/10 hover:border-cyan-500/30 group backdrop-blur-md shadow-lg hover:shadow-cyan-500/10 z-50"
                >
                    <span className="hidden md:block text-[10px] font-black uppercase tracking-[0.2em]">Guide</span>
            ...
            {mode === 'preview' && (
                <div className="flex p-1 bg-white/5 rounded-xl border border-white/5 relative z-10">
                    <button
                        ...
                        className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${inputType === 'text'
                    ...
                    <button
                        ...
                        className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${inputType === 'drawing'
                            ? 'bg-white text-slate-950 shadow-lg scale-105'
                            : 'text-slate-500 hover:text-white'
                            }`}
                    >
                ...
                <div className={`flex flex-wrap justify-center gap-3 w-full max-w-4xl px-4 transition-all duration-500 ${mode !== 'preview' ? 'opacity-50 pointer-events-none blur-[2px]' : ''}`}>
                    {fonts.map((f) => (
                        <button
                            ...
                            disabled={mode !== 'preview'}
                            className={`px-5 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all border whitespace-nowrap flex-shrink-0 ${selectedFont === f.name
